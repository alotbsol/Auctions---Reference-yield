...
def scenario3_german_auctions_multi(iterations=1):
...
def multi_german_auctions():
    ...
    it_per_cpu = iterations/cpu_no
    print(it_per_cpu)
    print("cpus", cpu_no)

    Parallel(n_jobs=cpu_no, require='sharedmem')(delayed(scenario3_german_auctions_multi)
                                                 in range(cpu_no))
